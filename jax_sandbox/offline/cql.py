...
from jax_sandbox.common.utils import opt_class, sample_multiple

class CQLState(NamedTuple):
    actor_params: hk.Params
    critic_params: hk.Params
    target_critic_params: hk.Params
    log_alpha_params: Optional[hk.Params]
    log_cql_alpha_params: Optional[hk.Params]
    
    actor_opt_state: optax.OptState
    critic_opt_state: optax.OptState
    log_alpha_opt_state: Optional[optax.OptState]
    log_cql_alpha_opt_state: Optional[optax.OptState]
...
class CQL:
    def __init__(self, cfg):
        ...
        if cfg.use_lagrange:
            cql_log_alpha = jnp.asarray(0.0, dtype=jnp.float32)
            cql_alpha_opt = opt_class(cfg.optim)(learning_rate=cfg.cql_alpha_lr)
            cql_alpha_opt_state = cql_alpha_opt.init(cql_log_alpha)
        else:
            cql_log_alpha = None
            cql_alpha_opt_state = None
            
        self._state = CQLState(
            actor_params=actor_params,
            critic_params=critic_params,
            target_critic_params=target_critic_params,
            log_alpha_params=log_alpha,
            log_cql_alpha_params=cql_log_alpha,
            actor_opt_state=actor_opt_state,
            critic_opt_state=critic_opt_state,
            alpha_opt_state=alpha_opt_state,
            log_cql_alpha_opt_state=cql_alpha_opt_state,
            rng_key=state_key
        )
        ...
        # cql hparams
        n_cql_samples = cfg.n_cql_samples
        ...
        def critic_loss_fn(critic_params: hk.Params,
            ...
            # split keys to get what we want
            sac_key, cql_key1, cql_key2, cql_key_uniform = jax.random.split(key, 4)
            
            # get targets
            next_action_dist = actor.apply(actor_params, batch.next_states)
            next_actions = next_action_dist.sample(seed=sac_key)
            ...
            curr_q = jnp.minimum(curr_q1, curr_q2)
            
            # Q learning loss
            q_loss = jnp.mean(jnp.square(curr_q1 - next_v)) + jnp.mean(jnp.square(curr_q2, next_v))
            
            # ====== CQL component of loss ======
            policy_dist = actor.apply(actor_params, batch.states)
            policy_actions, policy_lps = sample_multiple(policy_dist, cql_key1, n_cql_samples) # (n, B, dim) -> (n * B, dim)
            policy_actions = policy_actions.reshape(-1, next_actions.shape[-1])
            policy_lps = policy_lps.reshape(-1, next_actions.shape[-1]).sum(-1)
            
            next_policy_actions, next_policy_lps = sample_multiple(next_action_dist, cql_key2, n_cql_samples) # (n, B, dim) -> (n * B, dim)
            next_policy_actions = next_policy_actions.reshape(-1, next_actions.shape[-1])
            next_policy_lps = next_policy_lps.reshape(-1, next_actions.shape[-1]).sum(-1)
            
            uniform_actions, uniform_lps = jax.random.uniform(cql_key_uniform, shape=policy_actions.shape, minval=min_action, maxval=max_action) # (n, B, dim) -> (n * B, dim)
            uniform_actions = uniform_actions.reshape(-1, next_actions.shape[-1])
            uniform_lps = uniform_lps.reshape(-1, next_actions.shape[-1]).sum(-1)
            
            repeated_shape = (n_cql_samples,) + (1,) * batch.states.ndim
            states = jnp.expand_dims(batch.states, 0).repeat(*repeated_shape).reshape(-1, batch.states.shape[1:])
            next_states = jnp.expand_dims(batch.next_states, 0).repeat(*repeated_shape).reshape(-1, batch.next_states.shape[1:])
            
            policy_q1, policy_q2 = critic.apply(critic_params, states, policy_actions) # (n * B,)
            policy_q = jnp.minimum(policy_q1, policy_q2).squeeze() - jax.lax.stop_gradient(policy_lps)
            
            next_policy_q1, next_policy_q2 = critic.apply(critic_params, next_states, next_policy_actions) # (n * B,)
            next_policy_q = jnp.minimum(next_policy_q1, next_policy_q2).squeeze() - jax.lax.stop_gradient(next_policy_lps)
            
            uniform_q1, uniform_q2 = critic.apply(critic_params, states, uniform_actions) # (n * B,)
            uniform_q = jnp.minimum(uniform_q1, uniform_q2).squeeze() - jax.lax.stop_gradient(uniform_lps)
            
            combined = jnp.concatenate([policy_q, next_policy_q, uniform_q], -1)
            lse_q = jax.nn.logsumexp(combined, axis=0, b=1.0 / (3 * n_cql_samples))
            
            cql_loss = jnp.mean(lse_q - curr_q) # TODO replace minimum with individual things and cat all
        ...
        def update(state: CQLState, batch: TransitionBatch, step: int) -> Tuple[CQLState, Dict]:
            del step
